...
apt install -y --no-install-recommends cups curl exfat-fuse firefox-esr gzip jwm minidlna nfs-kernel-server nginx-extras novnc ntfs-3g openssl php-fpm printer-driver-hpcups qbittorrent-nox resolvconf samba tar tigervnc-standalone-server unzip wireguard-tools wsdd
