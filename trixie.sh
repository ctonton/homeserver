...
if [ -z $time ] ; then
  ...
  [ -f /usr/share/zoneinfo/"$time" ] || exit 1
  ln -f -s /usr/share/zoneinfo/"$time" /etc/localtime
...
[[ $ramm -ge 1 ]] && pkg+=(cups-browsed ffmpeg firefox-esr jwm nginx-extras nmap novnc openssl php-fpm printer-driver-hpcups shellinabox sshpass sudo tigervnc-standalone-server)
apt install -y ${pkg[@]}
